import { CrossIcon, SearchIcon } from "@/public/icons/icons";
...
const Transaction = () => {
  const totalsell = useSelector((state: any) => state?.user?.eCommerceSales) || []
      ...
      <div className="h-[85%] flex flex-wrap justify-between gap-2 p-2 overflow-y-auto">
        {sale.length > 0 ? (
          sale.map((item:any, index:number) =>
            item.items.map((sold:any, index:number) => (
              <div
                key={index}
