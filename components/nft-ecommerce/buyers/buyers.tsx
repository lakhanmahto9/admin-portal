...
export const Buyers: React.FC = () => {
  ...
  const buyers = useSelector(
    (state: any) => state.getEcommerceBuyers?.data?.data || []
  );
  ...
  const buyerPurchasedproducts = (item: any) => {
    router.push(
      `/admin-dashboard/nft-ecommerce/buyer-purchased-products/${item._id}`
    );
  };
              ...
              <div className="relative w-full md:w-[32%] h-72" key={index}>
                ...
                >
                  <div className="flex gap-2 h-14 px-2">
                    <div
                      onClick={() => buyerPurchasedproducts(item)}
                      className={`w-1/2 ${
                        isDarkEnabled ? "bg-[#051139]" : "bg-[#025f92]"
                ...
                <div
                  className={`flex justify-center items-center gap-1 absolute w-[30%] h-8 bottom-[29%] left-[35%] px-2 ${
                    isDarkEnabled ? "bg-[#051139] border border-gray-600" : "bg-[#2c8993]"
                  } rounded-full`}
                >
