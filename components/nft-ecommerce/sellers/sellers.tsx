import {
  ...
  export const Sellers: React.FC = () => {
    ...
    // Fetch sellers when the component loads
    useEffect(() => {
      dispatch(getEcommerceSellersThunk());
    }, [dispatch]);
  
    // Update filtered sellers whenever the sellers data or search query changes
    useEffect(() => {
      if (sellers.length > 0) {
    ...
    const profile = (item: any) => {
      router.push(`/admin-dashboard/nft-ecommerce/seller-profile/${item?._id}`);
    };
            ...
            {filteredSellers.length > 0 ? (
              ...
              <div className="w-full h-full flex justify-center items-center">
                <div className="w-96 h-96 p-4 flex flex-col justify-center items-center">
                  ...
                  <p className="text-lg font-semibold">No Seller found</p>
                </div>
              </div>
            )}
          </div>
