...
export const FranchisePartnersList: React.FC = () => {
  ...
  // const franchisePartners = (id: string) => {
  //   router.push(`/admin-dashboard/nft-franchise/franchise-partners/${id}`);
  // };
  useEffect(() => {
    if (id) {
      callApiToPhuyer();
  ...
  const callApiToPhuyer = async () => {
    try {
      ...
      const result = await dispatch<any>(
        fetchParticularQPartner({ franchiseId: id })
      );
      if (result.payload?.fetchFranchiseParticularQPartner) {
        setBuyers(result.payload.fetchFranchiseParticularQPartner);
        setFilteredBuyers(result.payload.fetchFranchiseParticularQPartner);
      }
      setLoading(false);
    } catch (error) {
      console.log(error);
              ...
              >
                ...
                >
                  <div className="flex gap-2 h-14 px-2">
                    <div
                      // onClick={() => franchisePartners(item)}
