import { LeftIcon, SearchIcon, ThreedotIcon, VerifyIcon } from "@/public/icons/icons";
...
export const PhotographySellerDetails: React.FC = () => {
  ...
  const sellerPhotography = (item:any) =>{
    ...
    router.push(`/admin-dashboard/seller-photography/photography-list/${item._id}`)
  }

  const sellerProfile = (item:any) =>{
    localStorage.setItem("image",item.profile_pic);
    localStorage.setItem("sellername",item.name);
      ...
      <div className="w-full h-[88%] overflow-y-scroll flex flex-wrap justify-between p-2 gap-2">
        {phseller.map((item:any,index:number) => (
          <div className="relative w-[32%] h-72" key={index}>
            <div className="h-2/3 rounded-t-2xl bg-[#025f92] flex justify-center items-center">
              <div className="relative w-28 h-28 rounded-full border">
                ...
                <div className="absolute bottom-2 right-2">
                  <VerifyIcon color={item.isApproved?"#00ff00":"#ff0000"} height="20" width="20"/>
                </div>
              </div>
            </div>
            <div className="h-1/3 bg-[#084363] rounded-b-2xl py-5">
              <div className="flex gap-2 h-14 px-2">
                <div onClick={()=>sellerPhotography(item)} className="w-1/2 bg-[#025f92] cursor-pointer rounded-lg flex justify-center items-center">
            ...
            <div onClick={()=>sellerProfile(item)} className="absolute top-2 right-2 px-5 rounded-full w-auto h-7 bg-[#084363] cursor-pointer">
