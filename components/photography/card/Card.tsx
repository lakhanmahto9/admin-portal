...
import { getPhotographyCardDataApi } from "@/redux/api/photography/photographyapi";
...
export const Card: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getPhotographyCardDataApi();
        if (response.data) {
  ...
  const stats = [
...

    {
      ...
      since: "since last year",
      icon: <PhotoIcon color="#22219a" height="18" width="18" />,
    },

  
    {
      title: "Today's photographs",
