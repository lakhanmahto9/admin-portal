...
export const SellerProfile: React.FC = () => {
  const profile = useSelector(
    (state: any) => state.artmusicsellerprofile?.data?.seller
  );

  const profile1 = useSelector(
    (state: any) => state.artmusicsellerprofile?.data?.sellerAdress
  );
  ...
  const callApiToProfile = async () => {
    try {
      await dispatch<any>(artMusicSellerProile({ sellerId: id }));
    } catch (error) {
      console.log(error);
    }
  };
      ...
      >
        ...
        <div className="w-full h-[88%] overflow-y-scroll flex flex-col md:flex-row p-2 md:p-7 gap-4">
          <div className="w-full md:w-[40%] flex flex-col gap-4">
            ...
            <div
              className={`relative w-full h-1/2 rounded-lg px-16 py-2 ${
                isDarkModeEnable
                  ? "bg-[#051139] border border-gray-600"
                  : "bg-[#ebf6fd] border "
              }`}
