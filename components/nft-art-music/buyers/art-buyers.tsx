...
export const AllBuyerDetails: React.FC = () => {
  ...
  const callApiTofetchAllArtMusicBuyers = async () => {
    setLoading(true); // Start loading
    const result = await dispatch<any>(fetchAllArtMusicBuyers());
    if (result.payload?.success) {
      setBuyers(result?.payload?.data?.buyers);
    }
    setLoading(false); // Stop loading
  };

  useEffect(() => {
    callApiTofetchAllArtMusicBuyers();
  }, []);
    ...
    <>
      ...
      >
        ...
        <div className="w-full h-[88%] overflow-y-scroll flex flex-wrap justify-between p-2 gap-2">
          ...
          ) : buyers.length === 0 ? ( // Display message if no data is found
            <div className="w-full flex justify-center items-center h-full">
              <p className="text-xl" style={{ color: colors.text }}>
                No buyers found.
              </p>
